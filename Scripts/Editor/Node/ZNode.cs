//////////////////////////////////////////////////////////////////////////////////////////////////
/// Class: 	  ZNode
/// Purpose:  The base class for all nodes in the Node Editor. Has both core and custom types.
...
namespace ZEditor {
	...
	public class ZNode {
		...
		GUIStyle _defaultStyle, _selectedStyle, _xButtonStyle, _imgStyle;
		...
		public enum CORE_TYPE { ROOT, SINGLE_CHILD, MULTI_CHILD, LEAF, SUB_TREE }
	    public CORE_TYPE CoreType;

		public ZCustomNodeManager.CUSTOM_TYPE CustomNodeType;

		public ZNode(CORE_TYPE type, ZCustomNodeManager.CUSTOM_TYPE customType, ZNodeTree nodeTree, Rect wr) {
	        CoreType = type;
			CustomNodeType = customType;
			_nodeTree = nodeTree;
			_windowRect = wr;
			Name = "";
		...
		public void SetupSkinParams() {
			...
			if(CustomNodeType == ZCustomNodeManager.CUSTOM_TYPE.NONE) {
				...
				_imgStyle.normal.background = NodeEditor.SkinItem.GetCustomNodeImage(CustomNodeType) as Texture2D;
				_inspectorName = NodeEditor.SkinItem.GetCustomNodeInspectorName(CustomNodeType);
			}
		}
		...
		public void Draw() {
			...
			if(_isSelected && CoreType != CORE_TYPE.ROOT) {
	    ...
	    virtual public void Serialize(ref JSON nodeJS) {
			nodeJS["id"] = Name;
	        nodeJS["coreType"] = CoreType.ToString();
			nodeJS["customType"] = CustomNodeType.ToString();
