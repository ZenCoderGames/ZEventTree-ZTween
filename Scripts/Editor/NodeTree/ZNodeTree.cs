...
namespace ZEditor {
	...
	public class ZNodeTree {
		...
		ZNode CreateRoot(float posX, float posY) {
			_rootNode = new ZNodeRoot(this, new Rect(posX, posY, _nodeEditor.SkinItem.nodeWidth, _nodeEditor.SkinItem.nodeHeight), ZCustomNodeManager.CUSTOM_TYPE.NONE);
			_nodes.Add(_rootNode);
			return _rootNode;
		}

		public ZNode CreateNode(ZNode.CORE_TYPE type, float posX, float posY, JSON js) {
			Rect nodeRect = new Rect(posX, posY, _nodeEditor.SkinItem.nodeWidth, _nodeEditor.SkinItem.nodeHeight);
			ZNode node = null;
			if(type == ZNode.CORE_TYPE.SUB_TREE)
				node = new ZNodeSubTree(this, nodeRect, ZCustomNodeManager.CUSTOM_TYPE.NONE, js);
			_nodes.Add(node);
			return node;
		}

		public ZNode CreateCustomNode(ZCustomNodeManager.CUSTOM_TYPE type, float posX, float posY, JSON js) {
			Rect nodeRect = new Rect(posX, posY, _nodeEditor.SkinItem.nodeWidth, _nodeEditor.SkinItem.nodeHeight);
			ZNode node = ZCustomNodeManager.CreateCustomNode(type, this, nodeRect, js);
			_nodes.Add(node);
			return node;
		}
		...
		public void CreateConnection(ZNodeConnector inConnector, ZNodeConnector outConnector) {
			...
			if(inConnector.Node.CoreType == ZNode.CORE_TYPE.ROOT || inConnector.Node.CoreType == ZNode.CORE_TYPE.SINGLE_CHILD) {
		...
		public void Load(JSON js, string filePath) {
			...
			JSON[] nodeListJS = js.ToArray<JSON>("nodes");
			for(int i=0; i<nodeListJS.Length; ++i) {
				float posX = nodeListJS[i].ToFloat("posX");
				float posY = nodeListJS[i].ToFloat("posY");
				ZCustomNodeManager.CUSTOM_TYPE customNodeType = ZUtils.ParseEnum<ZCustomNodeManager.CUSTOM_TYPE>(nodeListJS[i].ToString("customType"));
				ZNode newNode = null;
				if(customNodeType != ZCustomNodeManager.CUSTOM_TYPE.NONE) {
					newNode = CreateCustomNode(customNodeType, posX, posY, nodeListJS[i]);
				}
				else {
					ZNode.CORE_TYPE nodeType = ZUtils.ParseEnum<ZNode.CORE_TYPE>(nodeListJS[i].ToString("coreType"));
					if(nodeType == ZNode.CORE_TYPE.ROOT)
						newNode = CreateRoot(posX, posY);
					else
						newNode = CreateNode(nodeType, posX, posY, nodeListJS[i]);
				}
