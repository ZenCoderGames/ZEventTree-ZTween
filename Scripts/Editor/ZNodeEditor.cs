...
namespace ZEditor {
	...
	public class ZNodeEditor:EditorWindow {
	    ...
	    static void Init() {
			ZNodeEditor window = (ZNodeEditor)EditorWindow.GetWindow (typeof (ZNodeEditor));
			window.Show();
	    ...
	    #region NODES
		public void CreateNode(ZNode.CORE_TYPE type) {
			_currentNodeTree.CreateNode(type, _nodeScrollView.x + (position.width - SkinItem.leftPanelWidth)/2, _nodeScrollView.y + (position.height - SkinItem.tabHeight)/2, null);
		}
			
		public void CreateCustomNode(ZCustomNodeManager.CUSTOM_TYPE type) {
			_currentNodeTree.CreateCustomNode(type, _nodeScrollView.x + (position.width - SkinItem.leftPanelWidth)/2, _nodeScrollView.y + (position.height - SkinItem.tabHeight)/2, null);
		}
